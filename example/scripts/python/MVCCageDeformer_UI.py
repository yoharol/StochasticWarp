...
import olm.ui
from MVCCageDeformer import olmMVCDeformer

def createCageDeformer(cage, targets, local, split):
	print("perform cage def:", cage, targets, local, split)
	results = olmMVCDeformer(cage=cage, targets=targets, local=local, split=split)
	print("generate a cage base:", results[0])
	print("and deformer(s):",results[1:])

class MVCCageDeformerUI():
	def __init__(self):
		self.name = "OLMMVCCageDeformer"
		self.title = "OLM MVC Cage Deformer"
		...
		self.frameLayout = cmds.frameLayout(label = "Object and Cage", width= 350)
		cmds.columnLayout()
	...
	def performCageDeformer(self):
		...
		if len(realTargets)>0 and cmds.objExists(cage):
			createCageDeformer(cage, realTargets, localBind, oneDeformerPerShape)
		else:
